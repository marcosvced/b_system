...
const b_system = (() => {
    ...
    const SELECTORS = {
        ...
        BODY_DARK: 'body[data-bsystem="dark"]',
    }
    ...
    const system = {
        ...
        changeTransparentHeaderOnScroll: () => {
            ...
            const onScroll = () => {
                const currentStatus = window.scrollY >= GLOBAL.HEADER_BODY_SCROLL
                if (isNavdrawerActive || currentStatus === previousStatus) {
                    return;
                }
                if (currentStatus) {
                    headerTransparent.classList.add(CSS_CLASS.HEADER_LIGHT)
                    headerTransparent.classList.remove(CSS_CLASS.HEADER_TRANSPARENT)
                } else {
                    headerTransparent.classList.add(CSS_CLASS.HEADER_TRANSPARENT)
                    headerTransparent.classList.remove(CSS_CLASS.HEADER_LIGHT)
                }

                const event = new CustomEvent(EVENTS.ON_TRANSPARENT_HEADER_CHANGE_BACKGROUND, {detail: {isTransparent: !currentStatus}})
        ...
        createHeaderNavDrawer: () => {
            ...
            toggleButton.onclick = () => {
                isNavdrawerActive = !isNavdrawerActive
                console.log(isNavdrawerActive)
                const event = new CustomEvent(EVENTS.ON_MOBILE_NAV_TOGGLE, {detail: {isActive: isNavdrawerActive}})
                document.dispatchEvent(event)
            ...
            swiperList.forEach((swiper) => {
                ...
                if (slidesPerViewSm || slidesPerViewMd || slidesPerViewLg || slidesPerViewXl || slidesPerViewXxl) {
                    if (slidesPerViewSm) {
                        config.breakpoints = {
                            ...config.breakpoints,
                            [breakpointList.sm]: {slidesPerView: slidesPerViewSm}
                        }
                    }
                    if (slidesPerViewMd) {
                        config.breakpoints = {
                            ...config.breakpoints,
                            [breakpointList.md]: {slidesPerView: slidesPerViewMd}
                        }
                    }
                    if (slidesPerViewLg) {
                        config.breakpoints = {
                            ...config.breakpoints,
                            [breakpointList.lg]: {slidesPerView: slidesPerViewLg}
                        }
                    }
                    if (slidesPerViewXl) {
                        config.breakpoints = {
                            ...config.breakpoints,
                            [breakpointList.xl]: {slidesPerView: slidesPerViewXl}
                        }
                    }
                    if (slidesPerViewXxl) {
                        config.breakpoints = {
                            ...config.breakpoints,
                            [breakpointList.xxl]: {slidesPerView: slidesPerViewXxl}
                        }
                    }
                }
        ...
        darkModeSwitch: () => {
            ...
            if (switchInput) {
                ...
                switchInput.onclick = ($event) => {
                    ...
                    if (isChecked) {
                        body.setAttribute('data-bsystem', 'dark')
                        return
                    }
                    body.removeAttribute('data-bsystem')
                }
            }
        },
    ...
    function initBootstrap() {
        ...
        const myOffcanvas = document.querySelector('#offcanvasNavbar')
