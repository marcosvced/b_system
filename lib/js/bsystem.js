...
const system = {
    ...
    setBodyTopSpace: () => {
        const fontSizeBase = window.getComputedStyle(document.querySelector('html'), null).getPropertyValue('font-size')
        ...
        if (customSpace && 'number' === typeof Number.parseInt(customSpace)) {
            ...
            const clientHeight = contactBar ? contactBar.clientHeight + header.clientHeight : header.clientHeight
        ...
        if (contactBar) {
            const size = window.getComputedStyle(contactBar, null).getPropertyValue('border-bottom-width')
            header.style = `--top: ${(contactBar.clientHeight + (size ? Number.parseInt(size) : 0)) / Number.parseInt(fontSizeBase)}rem`
        }
    ...
    initSwiper: () => {
        ...
        swiperList.forEach((swiper) => {
            ...
            console.log(config.allowTouchMove)
            if (loop) {
                config.loop = true
    ...
    dropdownMultilevel: () => {
        document.querySelectorAll('.dropdown-menu span').forEach(function (element) {
    ...
    setDarkModeByQueryParam: () => {
        ...
        if (theme) {
            document.querySelector('body').removeAttribute('data-b-system-theme')
            document.querySelector('body').setAttribute('data-b-system-theme', theme)
        }
    ...
    dropdowns: () => {
        ...
        document.querySelectorAll('[data-bs-toggle="dropdown"]').forEach((dropdown) => {
            ...
            dropdown.parentElement.addEventListener('mouseover', () => {
                document.querySelectorAll('[data-bs-toggle="dropdown"]').forEach((element)=>  bootstrap.Dropdown.getInstance(element).hide())
                instance.show()
            })
